...
function sendVal(a) {
  ...
  localStorage.setItem(
    'lsEfectivo',
    document.getElementById('txt_pagoCash').value
  );
  localStorage.setItem(
    'lsVuelto',
    document.getElementById('vuelto').innerHTML.substr(11)
  );
...
function sendRef(a) {
  localStorage.setItem(
    'lsTarjetaCredito',
    document.getElementById('total_pagar').innerHTML.substr(16)
  );
...
function validarMonto() {
  ...
  if ($('.valPago').val() >= totalXPagar) {
    ...
    document.getElementById('vuelto').textContent =
      'Su vuelto: ' +
      (totalXPagar * -1 + parseFloat($('.valPago').val())).toString();
...
function facturar() {
  ...
  $(t.rows().data()).each(function(i, item) {
    ...
    objetoDetalleFactura.detalle = `${item[1]}, ${item[3]}, ${item[5]}, ${
      item[7]
    }`;
    objetoDetalleFactura.precioUnitario = precioUnitario;
    objetoDetalleFactura.montoTotal = parseFloat(
      (
        objetoDetalleFactura.precioUnitario * objetoDetalleFactura.cantidad
      ).toFixed(5)
    );
    objetoDetalleFactura.montoDescuento = 0;
    objetoDetalleFactura.naturalezaDescuento = 'No aplican descuentos';
    objetoDetalleFactura.subTotal = parseFloat(
      (
        objetoDetalleFactura.montoTotal - objetoDetalleFactura.montoDescuento
      ).toFixed(5)
    );
    // exoneracion
    ...
    facturaCli.totalVenta = parseFloat(
      (facturaCli.totalVenta + objetoDetalleFactura.montoTotal).toFixed(5)
    );
    facturaCli.totalDescuentos = parseFloat(
      (
        facturaCli.totalDescuentos + objetoDetalleFactura.montoDescuento
      ).toFixed(5)
    );
    facturaCli.totalImpuesto = parseFloat(
      (facturaCli.totalImpuesto + montoTotalImpuestosLinea).toFixed(5)
    );
    //
  ...
  if ($('#txt_pagoCash').val() == null) facturaCli.montoEfectivo = '0';
  else facturaCli.montoEfectivo = $('#txt_pagoCash').val();
  if ($('#total_pagar').text() == null) facturaCli.montoTarjeta = '0';
  else
    facturaCli.montoTarjeta = $('#total_pagar')
      .text()
      .substring(16);
  ...
  facturaCli.totalGravado = parseFloat(
    (facturaCli.totalServGravados + facturaCli.totalMercanciasGravadas).toFixed(
      5
    )
  );
  facturaCli.totalExento = parseFloat(
    (facturaCli.totalServExentos + facturaCli.totalMercanciasExentas).toFixed(5)
  );
  facturaCli.totalVenta = parseFloat(
    (facturaCli.totalGravado + facturaCli.totalExento).toFixed(5)
  );
  // total venta neta.
  facturaCli.totalVentaneta = parseFloat(
    (facturaCli.totalVenta - facturaCli.totalDescuentos).toFixed(5)
  );
  // total comprobante.
  facturaCli.totalComprobante = parseFloat(
    (facturaCli.totalVentaneta + facturaCli.totalImpuesto).toFixed(5)
  );
  //

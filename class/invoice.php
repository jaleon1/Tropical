    ...
    public static function Create($transaccion){
        try {
            ...
            $sql='SELECT s.email_name, s.email_subject, s.email_SMTPSecure, s.email_Host, s.email_SMTPAuth, s.email_user, s.email_password, s.email_ssl, 
                    s.email_smtpout, s.email_port, s.email_body, s.email_logo, s.html, s.email_footer, e.numTelefono, e.idTipoIdentificacion, e.identificacion, p.provincia, c.canton
                FROM smtpXEntidad s
                    INNER JOIN clienteFE e ON s.idEntidad = e.idBodega
            ...
            if ($data){
                ...
                $idTipoIdentificacion = $data[0]["idTipoIdentificacion"];
            ...
            $sql='SELECT xml FROM historicoComprobante
            ...
            //tipo moneda
            $signo= '¢';
            switch($transaccion->idCodigoMoneda){
                case 55:
                    $signo = '¢';
                break;
                case 72:
            ...
            //
            $InvoicePrinter = new InvoicePrinter("A4", $signo, "es");    
            /* Header Settings */
            ...
            $InvoicePrinter->setDate( date('d')." de ".$meses[date('n')-1]. ", ".date('Y') .  " ".date('h:ia') );
            //
            $lang['legal_document'] =  $idTipoIdentificacion == 1  ?  'Cédula Física' : 'Cédula Jurídica';
            ...
            //
            if($transaccion->datosReceptor->nombre=='default'){
                $transaccion->datosReceptor->nombre='Sin receptor';
